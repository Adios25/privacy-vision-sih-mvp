...
const state = { tabId: null, windowId: null, scan: null, payload: null, plan: null, pendingHighRisk: [], receipt: [], profile: null, requestStarted: 0 };
...
async function ensureContentScript(tabId) {
  try { await promiseCall(api.tabs, 'sendMessage', tabId, { type: 'PV_PING' }); }
  catch { await promiseCall(api.scripting, 'executeScript', { target: { tabId }, files: ['content.js'] }); }
}
...
function renderScan() {
  ...
  $('#plan-results').classList.add('hidden'); $('#execution-results').classList.add('hidden'); $('#confirmation').classList.add('hidden');
...
async function scanPage() {
  ...
  try {
    ...
    state.scan = response.data; state.payload = buildPayload(response.data, redacted, vision, detections); state.plan = null; state.pendingHighRisk = []; state.receipt = [];
    renderScan();
    renderPlan(createLocalPlan(state.payload.page));
...
function renderPlan(response) {
  state.plan = response;
  $('#provider').textContent = `${response.provider} · ${response.model}`;
  $('#planner-message').textContent = response.message || 'The planner returned a schema-constrained plan from sanitized context.';
  $('#action-list').innerHTML = response.actions.map((action, index) => `<div class="action-item"><code>${String(index + 1).padStart(2, '0')}</code><div><strong>${action.type}</strong><small>${action.targetId || action.reason || action.message || ''} ${action.placeholder || ''}</small></div><span class="action-state">planned</span></div>`).join('');
  $('#server-ms').textContent = `${response.metrics?.serverMs || 0} ms`; $('#model-ms').textContent = `${response.metrics?.modelMs || 0} ms`;
  $('#e2e-ms').textContent = response.provider === 'local' ? 'Local' : `${response.metrics?.e2eMs || 0} ms`;
  $('#plan-results').classList.remove('hidden');
  $('#execute').disabled = false;
  setBoundary(response.provider === 'local' ? 'Local plan ready' : 'Server plan received', `${response.actions.length} schema-constrained actions prepared by ${response.provider}.`, 'safe');
}
...
function renderReceipt(receipt) {
  ...
  $('#receipt-list').innerHTML = receipt.map((item, index) => `<div class="action-item"><code>${String(index + 1).padStart(2, '0')}</code><div><strong>${item.type} ${item.targetId || ''}</strong><small>${item.reason || ''}</small></div><span class="action-state" data-state="${item.status}">${item.status}</span></div>`).join('');
...
async function restoreSession() {
  ...
  if (state.receipt.length) renderReceipt(state.receipt);
...
async function execute(actions, allowHighRisk) {
  ...
  renderReceipt(response.data.receipt);
  await persistSession();
...
async function executeSafeActions() {
  ...
  $('#execute').disabled = true; setBoundary('Validating actions locally', 'Targets, state, field purpose, and risk are being checked before execution.', 'working');
  try {
    const safe = state.plan.actions.filter((action) => action.type !== 'CLICK' || !action.highRisk);
    state.pendingHighRisk = state.plan.actions.filter((action) => action.type === 'CLICK' && action.highRisk);
    if (!state.pendingHighRisk.length && state.plan.submissionTargetId) {
      state.pendingHighRisk = [{ type: 'CLICK', targetId: state.plan.submissionTargetId, highRisk: true }];
    }
    const receipt = await execute(safe, false);
    ...
    await persistSession();
  } catch (error) { setBoundary('Execution stopped', error.message, 'blocked'); }
...
async function confirmSubmission() {
  ...
  try {
    const receipt = await execute(state.pendingHighRisk, true);
...
async function clearSession() {
  ...
  state.scan = null; state.payload = null; state.plan = null; state.pendingHighRisk = []; state.receipt = [];
  await discardPersistedSession();
  $('#plan').disabled = true; $('#execute').disabled = true; $('#confirm').disabled = true;
  $('#scan-results').classList.add('hidden'); $('#plan-results').classList.add('hidden'); $('#execution-results').classList.add('hidden'); $('#confirmation').classList.add('hidden');
...
$('#execute').addEventListener('click', executeSafeActions);
